...
class Problem(object):
    ...
    def blank(self,number,mode=0,keyword = None):
        ...
        그리고 일률적으로 합친 텍스트를 Space 단위로 리스트로 나누어 담고, 다시 한 번 "\n"으로 나눈다.
        예 : "으로 한다.\n3호." => ["으로 한다.","3호."]
        이렇게 개행문자가 들어있던 문자는 "\n"없이 리스트 내 리스트로 저장이 된다.
        리스트 내 리스트를 "\n" + " " 가 포함된 스트링으로 해체하고, 다시 " "으로 나눈다.
        """
        self.Problem_text = All_Text(self.Whole_text)
        Problem_text_list1 = [word.split("\n") for word in self.Problem_text.split(" ")]
        for i in range(len(Problem_text_list1)):
            if type(Problem_text_list1[i]) == list:
                Problem_text_list1[i] = "\n ".join(Problem_text_list1[i]).split(" ")

        #itertools.chain 은 주소에 해당하는 리스트 내부에 들어있는 모든 주소를 일렬로 배열한다.
        # word.strip(" ")을 했을 때 None이 아닌 값만 받는다. Space string은 모두 걸러진다.  
        Problem_text_list2 = list(itertools.chain(*Problem_text_list1))
        Problem_text_list3 = [word.strip(" ") for word in Problem_text_list2 if word.strip(" ")]

        #모드 
        if mode == 0:
            Problem_text_list4 = random_blank(Problem_text_list3,number)
        elif mode == 1:
            Problem_text_list4 = random_blank_butwise(Problem_text_list3,number)

        #재조립  
        self.Problem_text = " ".join(Problem_text_list4)   
...
def random_blank_butwise(the_list,number):
    ...
    for i in range(number):
        ...
        while sublist[i] in (common + item_symbol + paragraph_symbol + [" ","\n"]):
